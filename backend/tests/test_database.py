...
def test_sqlite_foreign_keys():
    ...
    msg = Message(session_id="non-existent-session-id", role="user", content="Hello")
    db.add(msg)
...
def test_orm_relationships():
    ...
    assert db_session.messages[0].content == "Hello"
    assert db_session.messages[0].session == db_session
    db.close()
